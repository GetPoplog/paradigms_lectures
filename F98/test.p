...
define exam1(n);
    let i in
        ...
        for i from 1 to n do
            ...
            pr('Question 3\n');
            pr('\nWith the definition of lookup from assignment 2\n');
            pr('evaluate the following Scheme Expression\n\n');
            pr('    (lookup '); pr(random(6)); pr(' \'');
...
define exam2(n);
    let i in
        pr('\^L');
        for i from 1 to n do
            pr('\n\n\nCMPSCI 287 Class Test   15OCT\n\nNAME\n\nSTUDENT ID\n\n');
            printf('Test ID: %p,  ranseed = %p\n\n\n', [%i,ranseed%]);

            pr('Question 1\n');
            pr('Evaluate the following Scheme Expression\n\n');

            printf(q21, [% oneof([car cdr]), oneof([car cdr]),
                          random(10), random(10),random(10), random(10),
                          random(10)%]);

            pr('Question 2\n');

            pr('With the definition of *_rel from assignment 3\n');
            pr('evaluate the following Scheme Expression\n\n');
            pr('(*_rel\n    \''); pr_scm(perm_random());
            pr('\n    \'');
            pr_scm(perm_random());
            ...
            printf(q22, [% oneof([car cadr '(lambda (x) (+ (car x) 1))']),
                      perm_random()    %]);
